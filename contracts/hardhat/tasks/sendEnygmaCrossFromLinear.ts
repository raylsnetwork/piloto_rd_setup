...
task('sendEnygmaCrossFromLinear', 'Sends enygma from one private ledger to another, on behalf of a user using a wallet with allowance')
    ...
    .setAction(
        async (
            ...
            try {
                ...
                const tx = await token.connect(signer).linearCrossTransferFrom(
                    taskArgs.from,
                    taskArgs.to,
                    amount,
                    chainId,
