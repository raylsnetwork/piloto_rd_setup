...
task('unfreeze-token', 'Unfreezes a token for a list of participants')
    .addParam("participants", "The participants to unfreeze the token for (comma separated)")
    .addParam("resourceId", "The resourceId of the token to unfreeze")
    .setAction(async (taskArgs, hre) => {
        ...
        const tx = await TokenRegistry.unfreezeToken(hre.ethers.keccak256(hre.ethers.toUtf8Bytes(resourceId)), participants.split(",").map(Number));
        const receipt = await tx.wait();
        if (receipt?.status === 1) {
            console.log('Token unfrozen');
        }
