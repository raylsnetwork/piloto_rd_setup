...
describe('E2E Tests: Enygma LinearCrossTransferFrom', function () {
  ...
  it('Should not Cross transfer A -> B, from another wallet without allowance', async function () {
    ...
    const tx = tokenOnPLASignedByOtherWallet.linearCrossTransferFrom(signerA.address, signerB.address, 10, chainIdB, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
  ...
  it('Cross transfer A -> B, from another wallet with allowance', async function () {
    ...
    const tx = await tokenOnPLASignedByOtherWallet.linearCrossTransferFrom(signerA.address, signerB.address, 10, chainIdB, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);
    ...
    const txToRevert = tokenOnPLASignedByOtherWallet.linearCrossTransferFrom(signerA.address, signerB.address, 10, chainIdB, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbB?.balance_finalised !== BigInt(10).toString()) return false;
          // If the r value is the same as in the previous poll, we assume it has stabilized.
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma(BigInt(10).toString(), rB);
  ...
  it('Should not Cross transfer B -> A, from another wallet without allowance', async function () {
    ...
    const tx = tokenOnPLBSignedByOtherWallet.linearCrossTransferFrom(signerB.address, signerA.address, 10, chainIdA, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
  ...
  it('Cross transfer B -> A, from another wallet with allowance', async function () {
    ...
    const tx = await tokenOnPLBSignedByOtherWallet.linearCrossTransferFrom(signerB.address, signerA.address, 5, chainIdA, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
    const receipt = await tx.wait();
    ...
    // retrying the same transaction should revert due to no allowance
    const txToRevert = tokenOnPLBSignedByOtherWallet.linearCrossTransferFrom(signerB.address, signerA.address, 5, chainIdA, ethers.encodeBytes32String(""), ethers.ZeroAddress, "0x");
    await expect(txToRevert).to.be.revertedWithCustomError(
      tokenOnPLBSignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );
  ...
  it('Cross transfer A -> B, with a call to ArbitraryCallable', async function () {
    ...
    const tx = await tokenOnPLASignedByOtherWallet.linearCrossTransferFrom(signerA.address, signerB.address, 10, chainIdB, encodedCallables[0].resourceId, encodedCallables[0].contractAddress, encodedCallables[0].payload);
    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);
    ...
    // retrying the same transaction should revert due to no allowance
    const txToRevert = tokenOnPLASignedByOtherWallet.linearCrossTransferFrom(signerA.address, signerB.address, 10, chainIdB, encodedCallables[0].resourceId, encodedCallables[0].contractAddress, encodedCallables[0].payload);
    await expect(txToRevert).to.be.revertedWithCustomError(
      tokenOnPLASignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );
