...
describe('E2E Tests: Enygma 1->4 concurrency + mint and burn in pending flow', function () {
  ...
  it('Cross transfer A -> B, C, D, E', async function () {
    const initialBlockNumber = await providerCC.getBlockNumber();

    const tx = await tokenOnPLA.crossTransfer([signerB.address, signerC.address, signerD.address, signerE.address], [5, 5, 5, 5], [chainIdB, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    ...
    expect(balance).to.be.equal(980);
    ...
    // Check deploy for destination D
    ...
    // Check deploy for destination E
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            ...
            if (balanceOnPlE == BigInt(5)) return true;
            return false;
          } catch (e) {
            return false;
          }
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          if (enygmaOnDbA?.balance_finalised !== '980') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('980', rA);
  ...
  it('Cross transfer A -> B, C, D, E and B -> A, C, D, E in the same block', async function () {
    const initialBlockNumber = await providerCC.getBlockNumber();

    // Transfers from A to B, C, D
    const txAtoB = await tokenOnPLA.crossTransfer([signerB.address, signerC.address, signerD.address, signerE.address], [100, 2, 2, 2], [chainIdB, chainIdC, chainIdD, chainIdE], [[], [], [], []]);

    // Transfers from B to A, C, D
    const txBtoA = await tokenOnPLB.crossTransfer([signerA.address, signerC.address, signerD.address, signerE.address], [1, 1, 1, 1], [chainIdA, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            ...
            return balanceOnPlA == BigInt(875); // 980 - 106 + 1 = 875
          } catch (e) {
            return false;
          }
        },
    ...
    expect(
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            const balanceOnPlB = await tokenOnPLB.balanceOf(signerB.address);
            return balanceOnPlB == BigInt(101); // 5 + 100 - 4 = 101
          } catch (e) {
            return false;
          }
        },
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbA?.balance_finalised !== '875') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('875', rA);
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbB?.balance_finalised !== '101') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('101', rB);
  ...
  it('Cross transfer A -> B, C, D, E and B -> A, C, D, E with a subsequent transfer B -> A, C, D, E', async function () {
    let initialBlockNumber = await providerCC.getBlockNumber();

    // Initial cross transfers
    const txAtoB = await tokenOnPLA.crossTransfer([signerB.address, signerC.address, signerD.address, signerE.address], [3, 3, 3, 3], [chainIdB, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    const txBtoA = await tokenOnPLB.crossTransfer([signerA.address, signerC.address, signerD.address, signerE.address], [2, 2, 2, 2], [chainIdA, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    ...
    if (!eventLogAtoB) throw new Error('crossTransferReferenceId event not found for A -> (B, C, D, E)');
    ...
    if (!eventLogBtoA) throw new Error('crossTransferReferenceId event not found for B -> (A, C, D, E)');
    ...
    // - On tokenOnPLB, tokenOnPLC, tokenOnPLD, and tokenOnPLE the status should be RECEIVED (2)
    await pollCondition(
      async (): Promise<boolean> => {
        ...
        return statusAtoBOnPLA == 1n && statusAtoBOnPLB == 2n && statusAtoBOnPLC == 2n && statusAtoBOnPLD == 2n && statusAtoBOnPLE == 2n;
      },
      1000, // Check every 1 second
      600 // Timeout after 600 seconds
    );
    LogForTest(`✅ ReferenceIds Status for A -> (B, C, D, E) verified`);

    // Poll for B -> (A, C, D, E):
    // - On tokenOnPLB the status should be SENT (1)
    // - On tokenOnPLA, tokenOnPLC, tokenOnPLD, and tokenOnPLE the status should be RECEIVED (2)
    await pollCondition(
      async (): Promise<boolean> => {
        ...
        return statusBtoAOnPLB == 1n && statusBtoAOnPLA == 2n && statusBtoAOnPLC == 2n && statusBtoAOnPLD == 2n && statusBtoAOnPLE == 2n;
      },
      1000,
      600
    );
    LogForTest(`✅ ReferenceIds Status for B -> (A, C, D, E) verified`);

    // Additional transfer from B to A, C, and D
    const tx = await tokenOnPLB.crossTransfer([signerA.address, signerC.address, signerD.address, signerE.address], [1, 1, 1, 1], [chainIdA, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    ...
    expect(
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            const balanceOnPlA = await tokenOnPLA.balanceOf(signerA.address);
            return balanceOnPlA == BigInt(866);
          } catch (e) {
            return false;
          }
        },
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbA?.balance_finalised !== '866') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('866', rA);
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbB?.balance_finalised !== '92') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('92', rB);
  ...
  it('Mint in PL A, Transfer 5 from PL B to PL A and C, D, E Burn 5 in PL A, Transfer 1 from PL B to PL A and C, D, E', async function () {
    ...
    LogForTest(`🛠️ Transferring 5 Enygmas from PL B to PL A, C, and D and E`);
    const transferTxBtoA = await tokenOnPLB.crossTransfer([signerA.address, signerC.address, signerD.address, signerE.address], [5, 5, 5, 5], [chainIdA, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    const transferReceiptBtoA = await transferTxBtoA.wait();
    expect(transferReceiptBtoA?.status).to.be.equal(1);
    LogForTest(`✅ Transferred 5 Enygmas from PL B to PL A, C, and D and E`);
    ...
    LogForTest(`🛠️ Transferring 1 Enygma from PL B to PL A, C, and D and E`);
    const transferTxBtoA2 = await tokenOnPLB.crossTransfer([signerA.address, signerC.address, signerD.address, signerE.address], [1, 1, 1, 1], [chainIdA, chainIdC, chainIdD, chainIdE], [[], [], [], []]);
    const transferReceiptBtoA2 = await transferTxBtoA2.wait();
    expect(transferReceiptBtoA2?.status).to.be.equal(1);
    LogForTest(`✅ Transferred 1 Enygma from PL B to PL A, C, and D and E`);
    ...
    expect(
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            const balanceOnPlA = await tokenOnPLA.balanceOf(signerA.address);
            return balanceOnPlA == BigInt(877);
          } catch (e) {
            return false;
          }
        },
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbA?.balance_finalised !== '877') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('877', rA);
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbB?.balance_finalised !== '68') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('68', rB);
