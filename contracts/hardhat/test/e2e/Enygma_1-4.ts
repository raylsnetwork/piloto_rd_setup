...
describe('E2E Tests: Enygma 1->4', function () {
  ...
  it('Cross transfer A -> B, C, D, E', async function () {
    ...
    const tx = await tokenOnPLA.crossTransfer([signerB.address, signerC.address, signerD.address, signerE.address], [1, 2, 3, 4], [chainIdB, chainIdC, chainIdD, chainIdE], [[], [], [], []]);

    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);

    const balance = await tokenOnPLA.balanceOf(signerA.address);
    expect(balance).to.be.equal(990); // Remaining balance after transferring 1+2+3+4=10
    ...
    if (useMongoDbChecks) {
      ...
      LogForTest(`✅ Checking balance on Enygma destinantion D`);
      ...
      LogForTest(`✅ Checking balance on Enygma destinantion E`);
    }
