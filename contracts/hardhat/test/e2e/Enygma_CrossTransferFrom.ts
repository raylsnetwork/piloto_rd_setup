...
describe('E2E Tests: Enygma CrossTransferFrom', function () {
  ...
  it('Should not Cross transfer A -> B, from another wallet without allowance', async function () {
    const tokenOnPLASignedByOtherWallet = tokenOnPLA.connect(signerOtherA);
    const tx = tokenOnPLASignedByOtherWallet.crossTransferFrom(signerA.address, [signerB.address], [10], [chainIdB], [[]]);

    await expect(tx).to.be.revertedWithCustomError(
      tokenOnPLASignedByOtherWallet,
  ...
  it('Should not Cross transfer B -> A, from another wallet without allowance', async function () {
    const tokenOnPLBSignedByOtherWallet = tokenOnPLB.connect(signerOtherB);
    const tx = tokenOnPLBSignedByOtherWallet.crossTransferFrom(signerB.address, [signerA.address], [10], [chainIdA], [[]]);

    await expect(tx).to.be.revertedWithCustomError(
      tokenOnPLBSignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );

  }).timeout(5 * 60 * 1000);

  it('Cross transfer A -> B, from another wallet with allowance', async function () {
    const initialBlockNumber = await providerCC.getBlockNumber();
    const tokenOnPLASignedByOtherWallet = tokenOnPLA.connect(signerOtherA);
    ...
    const txAllowance = await tokenOnPLA.connect(signerA).approve(signerOtherA.address, 10);
    const receiptAllowance = await txAllowance.wait();
    expect(receiptAllowance?.status).to.be.equal(1);

    const tx = await tokenOnPLASignedByOtherWallet.crossTransferFrom(signerA.address, [signerB.address], [10], [chainIdB], [[]]);
    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);
    ...
    const allowanceAfter = await tokenOnPLA.allowance(signerA.address, signerOtherA.address);
    expect(allowanceAfter).to.be.equal(BigInt(0));

    // retrying the same transaction should revert due to no allowance
    const txToRevert = tokenOnPLASignedByOtherWallet.crossTransferFrom(signerA.address, [signerB.address], [10], [chainIdB], [[]]);
    await expect(txToRevert).to.be.revertedWithCustomError(
      tokenOnPLASignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );
  ...
  it('Cross transfer B -> A, from another wallet with allowance', async function () {
    ...
    const txAllowance = await tokenOnPLB.connect(signerB).approve(signerOtherB.address, 5);
    const receiptAllowance = await txAllowance.wait();
    expect(receiptAllowance?.status).to.be.equal(1);

    LogForTest(`🛠️ Sending some enygma from B to A`);

    const tx = await tokenOnPLBSignedByOtherWallet.crossTransferFrom(signerB.address, [signerA.address], [5], [chainIdA], [[]]);
    const receipt = await tx.wait();

    expect(receipt?.status).to.be.equal(1);

    // check remaining allowance and assert it is 0
    const allowanceAfter = await tokenOnPLB.allowance(signerB.address, signerOtherB.address);
    expect(allowanceAfter).to.be.equal(BigInt(0));

    // retrying the same transaction should revert due to no allowance
    const txToRevert = tokenOnPLBSignedByOtherWallet.crossTransferFrom(signerB.address, [signerA.address], [5], [chainIdA], [[]]);
    await expect(txToRevert).to.be.revertedWithCustomError(
      tokenOnPLBSignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );
  ...
  it('Should not Cross transfer A -> B, C, D, E, F, from another wallet with insufficient allowance', async function () {
    const tokenOnPLASignedByOtherWallet = tokenOnPLA.connect(signerOtherA);
    ...
    const tx = tokenOnPLASignedByOtherWallet.crossTransferFrom(
      signerA.address,
  ...
  it('Cross transfer A -> B, C, D, E, F, from another wallet with allowance', async function () {
    const initialBlockNumber = await providerCC.getBlockNumber();
    const tokenOnPLASignedByOtherWallet = tokenOnPLA.connect(signerOtherA);
    const initialBalanceA = await tokenOnPLA.balanceOf(signerA.address);
    const initialBalanceB = await tokenOnPLB.balanceOf(signerB.address);
    ...
    // grant allowance to signerOtherA
    const txAllowance = await tokenOnPLA.connect(signerA).approve(signerOtherA.address, 15);
    const receiptAllowance = await txAllowance.wait();
    expect(receiptAllowance?.status).to.be.equal(1);

    const tx = await tokenOnPLASignedByOtherWallet.crossTransferFrom(
      signerA.address,
      [signerB.address, signerC.address, signerD.address, signerE.address, signerF.address],
      [1, 2, 3, 4, 5],
      [chainIdB, chainIdC, chainIdD, chainIdE, chainIdF],
    ...
    const balance = await tokenOnPLA.balanceOf(signerA.address);
    expect(balance).to.be.equal(initialBalanceA - BigInt(15));
    ...
    // retrying the same transaction should revert due to no allowance
    const txToRevert = tokenOnPLASignedByOtherWallet.crossTransferFrom(signerA.address, [signerB.address], [5], [chainIdB], [[]]);
    await expect(txToRevert).to.be.revertedWithCustomError(
      tokenOnPLASignedByOtherWallet,
      'ERC20InsufficientAllowance',
    );
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          try {
            ...
            if (balanceOnPlB == initialBalanceB + BigInt(1)) return true;
            return false;
          } catch (e) {
            return false;
          }
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbA?.balance_finalised !== (initialBalanceA - BigInt(15)).toString()) return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma((initialBalanceA - BigInt(15)).toString(), rA);
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          // Check the balance and block number conditions.
          if (enygmaOnDbB?.balance_finalised !== (initialBalanceB + BigInt(1)).toString()) return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma((initialBalanceB + BigInt(1)).toString(), rB);
