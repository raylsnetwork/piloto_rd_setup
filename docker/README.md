...
# Deploy Contracts
## Commit Chain Deployment
NODE_CC_CHAIN_ID="123456"
PRIVATE_KEY_SYSTEM="0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"
RPC_URL_NODE_CC="http://example.com:8545"
WS_URL_NODE_CC="ws://example.com:8546"

## Deploy Privacy Ledger
COMMITCHAIN_CCDEPLOYMENTPROXYREGISTRY=0x1234567890abcdef1234567890abcdef12345678
RPC_URL_NODE_PL="http://localhost:8545"
NODE_PL_CHAIN_ID="994003236"
...
2. Execute o comando `make setup-env` para realizar a instalação do `npm`, `hardhat` e `foundry`
3. Execute os seguintes comandos para utilizar o gerador de DH Keys:

Use o comando `go build` para compilar seu arquivo. Você pode especificar o sistema operacional e a arquitetura de destino usando variáveis de ambiente, caso sejam diferentes do seu sistema atual. Por exemplo:
...
```
Agora você gerou o arquivo executável. Para executá-lo no Linux, basta usar o comando `./dhgen`.
...
O banco de dados MongoDB local também será provisionado neste momento. A Privacy Ledger será inicializada após 90 segundos para garantir o provisionamento completo do MongoDB.
...
2. Após iniciar a Privacy Ledger será necessário realizar o deploy dos contratos, para realizar o deploy dos contratos atualize as variáveis de ambiente na sessão `# Deploy Contracts` no arquivo `.env`
```bash
# Deploy Privacy Ledger
RPC_URL_NODE_PL="http:/localhost:8545"
...
```bash
deploy-privacy-ledger - Deploy the privacy ledger
