...
task('sendEnygmaCross', 'Sends enygma from one private ledger to another')
  ...
  .setAction(
    async (
      ...
      try {
        ...
        const tx = await token.connect(signer).crossTransfer(destinations, amounts, chainIds, allCallables);
