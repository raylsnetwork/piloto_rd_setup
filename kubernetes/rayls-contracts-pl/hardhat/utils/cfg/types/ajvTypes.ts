...
type IsValues<T> =
  false extends IsUnion<T> ? TypeEquality<keyof T, string> : false;
/** true if type is a properties type and Union is false, or type is a discriminator type and Union is true */
type IsRecord<T, Union extends boolean> =
  Union extends IsUnion<T>
                  ...
                  : true extends IsValues<Exclude<T, null>>
                    ...
                    : true extends IsRecord<Exclude<T, null>, false>
                      ...
                      : true extends IsRecord<Exclude<T, null>, true>
                        ? {
                            [K in keyof Exclude<T, null>]-?: Exclude<
                              T,
                              null
                            >[K] extends string
                              ? {
...
type Struct = Object | Array | { type: string };

type Object = {
  type: 'object';
  properties?: Record<string, unknown>;
  optionalProperties?: Record<string, Struct>;
  patternProperties?: Record<string, Struct>;
};
...
// ajvHelper combines all optional properties into the properties object
export function ajvHelper<T extends Struct>(schema: T): T {
  const parser = <T extends Struct>(schema: T): T => {
    if (schema.type !== 'object' && schema.type !== 'array') return schema;

    if (schema.type === 'object') {
      const { optionalProperties, properties, patternProperties, ...rest } =
        schema as Object;

      if (patternProperties) {
        const innerSchema = Object.values(patternProperties)[0];
        const k = Object.keys(patternProperties)[0];

        return {
          ...schema,
          patternProperties: { [k]: parser(innerSchema as Struct) }
        };
      }
      ...
      const parsedProperties = Object.entries(properties).reduce(
        (acc, [k, v]) => ({ ...acc, [k]: parser(v as Struct) }),
    ...
    if (schema.type === 'array') {
      const { items } = schema as Array;

      return {
        ...schema,
        items: Array.isArray(items) ? items.map(parser) : parser(items)
      } as T;
    }
  ...
  return parser(schema) as unknown as T;
}
