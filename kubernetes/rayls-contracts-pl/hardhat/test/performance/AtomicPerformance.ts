...
describe('Atomic Performance tests', function () {
  ...
  describe('AtomicTeleport from A to B single batch', function () {
    it('AtomicTeleport from A to B single batch', async function () {
      ...
      // await sendBatchTransactions(totalTxCount, amount, currentNonce, tokenOnPLA.teleportAtomic);
      await sendBatchTransactions(totalTxCount, amount, currentNonce, true);

      startTime = Date.now();
      const balanceUpdated = await pollUntilBalanceUpdated(tokenOnPLB, signerB.address, BigInt(amount * totalTxCount) + initialBalanceOnB);
      expect(balanceUpdated).to.be.true;
