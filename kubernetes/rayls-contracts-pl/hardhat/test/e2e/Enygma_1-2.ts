...
describe('E2E Tests: Enygma 1->2', function () {
  ...
  it('Cross transfer A -> B, C', async function () {
    ...
    const tx = await tokenOnPLA.crossTransfer([signerB.address, signerC.address], [1, 2], [chainIdB, chainIdC], [[], []]);

    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);

    const balance = await tokenOnPLA.balanceOf(signerA.address);
    expect(balance).to.be.equal(997); // Remaining balance after transferring 1+2=3
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          if (enygmaOnDbA?.balance_finalised !== '997') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('997', rA);
