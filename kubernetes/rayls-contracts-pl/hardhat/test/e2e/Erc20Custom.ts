...
import { CustomTokenExample, EndpointV1, ParticipantStorageV1, TokenExample, TokenRegistryV1 } from '../../../typechain-types';
...
describe('E2E Tests: Erc20 custom Handler (erc20)', function () {
  ...
  let tokenOnPLA: CustomTokenExample;
  ...
  const tokenName = `TokenCustom ${randHex}`;
  const tokenSymbol = `TC_${randHex}`;
  let tokenResourceId: string;
  const attestationUuid = '0x00000000000000000000000000000000000000000000000000000000686f6c61';
  const fundManagerId = 1010;
  const fundManagerAccount = ethers.Wallet.createRandom();
  ...
  before(async function () {
    ...
    try {
      ...
      const tokenErc20Custom = await hre.ethers.getContractFactory('CustomTokenExample', signerA);
      ...
      tokenOnPLA = await tokenErc20Custom.deploy(tokenName, tokenSymbol, fundManagerId, fundManagerAccount, endpointAddressA);
  ...
  //   try {
  //     const amount = 30;
  //     logger.debug(`${(new Date()).toISOString()} - Calling and awaiting 'teleport'...[tokenOnPLA=${await tokenOnPLA.getAddress()}, signerA.address=${signerA.address}, amount=${amount}, chainIdB=${chainIdB}]`);
  ...
  //   } catch (error) {
  //     logger.debug(`${(new Date()).toISOString()} - ERROR: ${error}`);
  //     // this will prevent the test to be successful
  //     throw error;
  //   }
  ...
  //     if (balanceOnABefore === balanceOnAAfter || wallet2Balance === 0n)
  //       return false;
  //     return true;
  //   },
  //     1000,
  ...
  //   expect(await pollCondition(
  //     async (): Promise<boolean> => {
  //       const balanceOnAAfter = await tokenOnPLA.balanceOf(wallet1);
  //       const balanceOnBAfter = await tokenOnPLB.balanceOf(wallet2);

  //       if (
  //         balanceOnBBefore === balanceOnBAfter ||
  //         balanceOnAAfter !== balanceOnABefore
  //       )
  //         return false;
  //       return true;
  //     },
  ...
  //   expect(await tokenOnPLA.balanceOf(wallet1)).to.be.equal(
  //     balanceOnABefore,
  //   );
  // },
  // ).timeout(180000);
  ...
  //   const tx = tokenOnPLA.teleportAtomic(
  //     signerB.address,
  //     amount,
  //     chainIdD,
  //   );
  ...
  //     async (): Promise<boolean> => {
  //       const balanceOnAAfter = await tokenOnPLA.balanceOf(signerA.address);
  //       if (balanceOnAAfter != balanceOnABefore)
  //         return false;
  //       return true;
  //     },
  //     1000,
