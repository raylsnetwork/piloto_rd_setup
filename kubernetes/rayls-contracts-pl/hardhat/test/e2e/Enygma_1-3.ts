...
describe('E2E Tests: Enygma 1->3', function () {
  ...
  it('Cross transfer A -> B, C, D', async function () {
    ...
    const tx = await tokenOnPLA.crossTransfer([signerB.address, signerC.address, signerD.address], [1, 2, 3], [chainIdB, chainIdC, chainIdD], [[], [], []]);

    const receipt = await tx.wait();
    expect(receipt?.status).to.be.equal(1);

    const balance = await tokenOnPLA.balanceOf(signerA.address);
    expect(balance).to.be.equal(994); // Remaining balance after transferring 1+2+3=6
    ...
    if (useMongoDbChecks) {
      ...
      await pollCondition(
        async (): Promise<boolean> => {
          ...
          if (enygmaOnDbA?.balance_finalised !== '994') return false;
      ...
      expect(
        await pollCondition(
          async (): Promise<boolean> => {
            ...
            const peddersens = await getPeddersenFromEnygma('994', rA);
